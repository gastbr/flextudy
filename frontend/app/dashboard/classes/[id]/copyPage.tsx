...
export default function ClassDetailPage({ params }: { params: { id: string } }) {
  const { id } = usePromise(params); // Usamos el alias
  ...
  // console.log("state", state.flextudy.currentUser.user_type_name)
  const { fetch: data, loading, error, execute: executeGetToCreate } = useGet(`/classes/class/${id}`);
  ...
  useEffect(() => {
    if (data) {
      setFormatedData({
        ...
        longDescription: data.class.subject,
        // schedule: [
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex items-center">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star
                              key={i}
                              className={`h-5 w-5 ${i < Math.floor(classDetails.rating)
